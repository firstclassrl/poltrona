...
export const Chat: React.FC = () => {
  ...
  const getChatPartnerInfo = (chat: { client_name: string; client_photo?: string; staff_name: string; staff_photo?: string }) => {
    ...
    return {
      name: chat.staff_name || professionalLabel,
                    ...
                    {isLoading ? (
                      ...
                      chats.map((chat) => (
                        <div
                          ...
                          className={`p-4 border-b border-gray-100 cursor-pointer hover:bg-gray-50 transition-colors touch-target focus:outline-none focus:ring-2 focus:ring-blue-500 ${activeChat?.id === chat.id ? 'bg-green-50 border-green-200' : ''
                            }`}
                        >
                ...
                <div className="flex-1 flex items-center justify-center bg-white rounded-b-xl">
                  <div className="text-center">
                    ...
                    <h3 className="text-lg font-semibold text-gray-900 mb-2">
                      {user?.role === 'client' ? `Chat con ${terminology?.professionalArticle() || 'il professionista'}` : 'Seleziona una conversazione'}
                    </h3>
                    <p className="text-gray-500">
                      {user?.role === 'client'
                        ? `${terminology?.professionalPlural() || 'I professionisti'} ti contatteranno qui per comunicazioni importanti e per rispondere alle tue domande.`
