-- Trigger per inviare email automaticamente quando un appuntamento viene cancellato
-- Questo trigger chiama la Supabase Edge Function send-email per inviare email via Resend
...
EXCEPTION
    WHEN OTHERS THEN
        RAISE NOTICE 'pg_net extension non disponibile o già installata: %', SQLERRM;
END $$;
...
<head>
    <meta charset="utf-8">
    <title>Appuntamento Annullato - ' || p_shop_name || '</title>
    <style>
...
<body>
    ...
    CASE WHEN p_client_email IS NOT NULL THEN '<div class="detail-row"><span><strong>Email:</strong></span><span>' || p_client_email || '</span></div>' ELSE '' END ||
    CASE WHEN p_client_phone IS NOT NULL THEN '<div class="detail-row"><span><strong>Telefono:</strong></span><span>' || p_client_phone || '</span></div>' ELSE '' END ||
...
BEGIN
    ...
    IF NEW.status = 'cancelled' AND (OLD.status IS NULL OR OLD.status != 'cancelled') THEN
        ...
        -- Ottieni URL Supabase dal progetto corrente
        -- NOTA: Sostituisci 'tlwxsluoqzdluzneugbe' con il tuo project_id Supabase
        -- Puoi trovarlo in Supabase Dashboard > Settings > API > Project URL
        v_supabase_url := 'https://tlwxsluoqzdluzneugbe.supabase.co';
        
        -- Ottieni la service_role_key dalle variabili d'ambiente di Supabase
        -- Questa chiave è disponibile automaticamente nelle Edge Functions
        -- Per i trigger SQL, usa la service_role_key dal dashboard
        -- NOTA: Sostituisci con la tua service_role_key da Supabase Dashboard > Settings > API
        v_supabase_anon_key := current_setting('app.settings.service_role_key', true);
        
        -- Se non disponibile, usa anon key (meno sicuro ma funziona)
        IF v_supabase_anon_key IS NULL OR v_supabase_anon_key = '' THEN
            -- Fallback: usa anon key (deve essere configurata come variabile custom)
            v_supabase_anon_key := current_setting('app.settings.anon_key', true);
        END IF;
        
        -- Se ancora non disponibile, esci (non possiamo chiamare la Edge Function senza auth)
        IF v_supabase_anon_key IS NULL OR v_supabase_anon_key = '' THEN
            RAISE LOG '⚠️ Service role key non configurata. Impossibile inviare email. Configura app.settings.service_role_key in Supabase.';
            RETURN NEW;
        END IF;
        ...
        IF v_shop_email IS NOT NULL AND v_shop_email != '' THEN
            ...
            EXCEPTION
                WHEN OTHERS THEN
                    RAISE LOG 'Errore invio email al negozio: %', SQLERRM;
            END;
        END IF;
        ...
        IF v_client_email IS NOT NULL AND v_client_email != '' THEN
            ...
            EXCEPTION
                WHEN OTHERS THEN
                    RAISE LOG 'Errore invio email al cliente: %', SQLERRM;
            END;
        END IF;
...
COMMENT ON FUNCTION public.notify_barber_on_appointment_cancellation() IS 'Funzione trigger per notificare il barbiere (in-app e email) quando un appuntamento viene cancellato. Invia email automaticamente via Supabase Edge Function.';
